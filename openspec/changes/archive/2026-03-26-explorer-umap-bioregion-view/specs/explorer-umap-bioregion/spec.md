# Explorer UMAP bioregion (bubble) view — archived delta copy

Canonical spec: `openspec/specs/explorer-umap-bioregion/spec.md` (merged at archive).
...
Two-dimensional positions for hit dots SHALL be computed with **UMAP** (`umap-js` or equivalent) from **per-hit** vectors: a **multi-hot** over the sorted union of bioregions in the current result set, plus a small **deterministic jitter** derived from the hit id so colocated identical profiles separate slightly. The client SHALL guard single-hit and degenerate cases with a non-throwing fallback layout.
