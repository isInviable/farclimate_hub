# pinboard-saved-searches (archived change delta)

## Purpose

Saved searches for a project are stored only in `human.saved_searches`. The pinboard SHALL surface the **same** list as the explorer filter `SavedSearchMenu` (same fetch, same project scope), without duplicating rows as `human.pins`.
...
- **WHEN** the user confirms delete on a saved-search card (or equivalent destructive control)
