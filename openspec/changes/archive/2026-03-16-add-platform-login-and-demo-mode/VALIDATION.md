...
- [ ] Open the app in an incognito window (or logged out).
- [ ] Visit `/`, `/explorer/explorer`, `/skills`, and any public reading/exploration route.
- [ ] Confirm pages render and navigation works without being asked to log in.
- [ ] In the explorer (Solutions) flow, confirm the explorer header shows "Demo mode" and "Sign in" (not signed-in state).

## 4.2 Authenticated session and persistence-capable actions

- [ ] Sign in via `/login` (or `/admin/login` for admin).
- [ ] In the explorer (Solutions) flow, confirm the explorer header shows the user email and "Log out" instead of "Demo mode" / "Sign in".
