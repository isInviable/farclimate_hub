# Explorer chat pin capture (change spec)

Delta for capability `explorer-chat-pin-capture` introduced by change `explorer-chat-pins-and-citations`. Defines saving full chat threads and selection-based text pins from corpus chat surfaces.
