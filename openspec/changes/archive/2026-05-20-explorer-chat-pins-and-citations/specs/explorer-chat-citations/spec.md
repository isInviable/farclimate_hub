# Explorer chat citations (change spec)

Delta for capability `explorer-chat-citations` introduced by change `explorer-chat-pins-and-citations`. Defines structured per-response citations from `/api/chat` and corpus-mode UI to view and open cited papers.
