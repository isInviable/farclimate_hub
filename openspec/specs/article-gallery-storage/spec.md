# article-gallery-storage Specification

## Purpose

Define how article images are persisted across three surfaces: a public `article-images` Supabase Storage bucket for the binaries, a `knowledge.document_images` Postgres table for the metadata, and the public SQL API wrappers that expose an aggregated `images jsonb` column to the explorer frontend.
