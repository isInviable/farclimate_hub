...
The PowerPoint wizard SHALL provide a first step that displays selected item previews, selected item count, context size, and generation instruction inputs. The selection validation rules MUST be consistent with the presentation structure endpoint limits for empty selections, selected-item count, and text-size limits.
...
- **AND** it MUST show context count/size information before generation
