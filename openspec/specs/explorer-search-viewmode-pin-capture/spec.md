# explorer-search-viewmode-pin-capture Specification

## Purpose

Define capturable-only pinning for explorer **list** and **Instagram** search viewmodes: **`CapturableBlock`**, per-hit **`PinArticleContext`**, and **`human.pins`**—with no legacy **`Pin`** wrapper.
## Requirements
