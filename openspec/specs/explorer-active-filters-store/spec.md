...
### Requirement: Sidebar chart filters reflect restored `filters` state

Bar-style facet controls (sector, climate impacts, biogeographical regions) SHALL keep their visible selection in sync with `FilterManager`’s `filters` map (e.g. a `selection` prop wired from `filters.sector` / `filters.hazards` / `filters.biogeographical_regions`) so **saved-search load** and other parent-driven updates show the correct checked state and status text, not only search results or the Pinia snapshot.

#### Scenario: Load saved search with sector selection

- **WHEN** a saved search is loaded that includes an enabled sector filter and a non-empty `filters.sector` map
- **THEN** the sector filter UI SHALL display the same selections as that map (including non-zero “options selected” when applicable)
