...
const formData = ref<AuxTheme | Record<string, any>>({} as AuxTheme);
...
function openEdit(row: AuxTheme) {
...
async function saveCreate() {
  ...
  try {
    ...
    isCreating.value = false;
    originalId.value = null;
    formData.value = {} as AuxTheme;
    await fetchThemes();
  } catch (e: any) {
    console.error(e);
    error.value = e.message ?? "Failed to create theme.";
  } finally {
    loading.value = false;
  }
}
...
async function deleteTheme() {
  ...
  try {
    ...
    console.error(e);
    error.value = e.message ?? "Failed to delete theme.";
  } finally {
    loading.value = false;
  }
}
...
<template>
  <UDashboardPanel id="admin-aux-themes">
    <template #header>
    ...
    <template #body>
      ...
      <UModal
        ...
        fullscreen
        :title="isCreating ? 'New theme' : (formData.name || 'Edit theme')"
      >
        <template #body>
          <div class="space-y-4">
      ...
      <UModal v-model:open="showDeleteConfirm" :title="`Delete theme: ${formData.name || 'this item'}`">
        <template #body>
          <UAlert
            color="error"
            variant="soft"
